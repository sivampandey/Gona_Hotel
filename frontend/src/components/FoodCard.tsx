...
export const FoodCard: React.FC<FoodCardProps> = ({ item }) => {
  ...
  return (
    <div className="group rounded-3xl bg-white border border-gray-200 overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
      
      {/* Food Image Header Container with 4:3 Aspect Ratio */}
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-gray-100">
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = '/images/restaurant/sada-thali-bhojan.webp';
          }}
        />
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        {/* Top Badges Bar: Pure Veg / Non Veg & Wishlist & Chef's Special */}
        <div className="flex items-center justify-between gap-2">
          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 border shadow-sm ${item.isVeg ? 'bg-green-50 border-green-600 text-green-700' : 'bg-red-50 border-red-600 text-red-700'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 rounded-full bg-gray-50 hover:bg-gray-100 text-gray-700 hover:text-red-500 transition-colors shadow-sm"
              title={wishlisted ? 'Remove from food wishlist' : 'Save to food wishlist'}
            >
        ...
        {/* Content Details: Prep Time, Rating, Title & Description */}
        ...
        <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
          ...
          {isItemAvailable ? (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold tracking-wider flex items-center gap-1.5 transition-all shadow-md cursor-pointer ${inCartQty > 0 ? 'bg-green-700 text-white hover:bg-green-800' : 'bg-[#0D3B29] hover:bg-[#134A35] text-white'}`}
            >
